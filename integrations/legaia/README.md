...
- completed generic observer foundations through protocol 1.5: safe runtime/executable identities, authoritative watched-generation reporting, native validity, backend-neutral execution-owner observations, bounded exact-instruction witnesses, stateless profile-scoped observation guards, guarded `read_regions`, an eight-record executable catalog, and bounded image-lifecycle pages. These changes are generic and contain no Legaia addresses or traversal logic.
...
- implemented the external headless observer, profile-driven scoped epoch, and transition-only manual watch mode. The real lifecycle is `town01 -> map01 -> town0c`; two fresh runtime processes accepted `legaia-na-scus94254-town0c-field-v1`, rejected old tokens in stable map01, and recovered stable town0c boundaries. The four-case live A/B found no material 2 Hz observer penalty and identified interpreter execution volume as the dominant measured map/town difference. One separately approved bounded read-only traversal is now safe, but none has been performed. See `docs/legaia-sdk/town0c-runtime-profile.md`, `docs/legaia-sdk/world-map-performance-analysis.md`, and `docs/legaia-sdk/actor-observation-readiness.md`.
...
The read-only scene/actor inspection surface is implemented under `integrations/legaia/inspector`; see `docs/legaia-sdk/town01-inspector.md`. The external headless observer is implemented under `integrations/legaia/observer`; its scoped town01 boundary and repeated town0c round-trip behavior are live-validated. A single bounded retail node traversal is safe only in a separately approved phase and has not occurred. RAM writes, actor correlation and generic runtime title checks remain out of scope.
