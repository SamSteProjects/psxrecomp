...
- implemented the external headless observer and profile-driven scoped epoch. Repeated retail town01 boundary-only passes remained stable despite unrelated global token churn. Actor traversal is still disabled because live scene exit and re-entry were not reached in the acceptance pass. See `docs/legaia-sdk/headless-runtime-observer.md`.
