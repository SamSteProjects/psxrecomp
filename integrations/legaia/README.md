...
- implemented the external headless observer, profile-driven scoped epoch, and transition-only manual watch mode. Repeated retail town01 boundary-only passes remained stable despite unrelated global token churn. The real lifecycle was confirmed as `town01 -> map01 -> town0c`; a read-only `town0c -> map01 -> town0c` round trip passed with explicit old-token rejection and a new observer epoch. Actor traversal remains disabled pending a revisioned town0c profile and separate approval. See `docs/legaia-sdk/headless-runtime-observer.md` and `docs/legaia-sdk/scene-transition-acceptance.md`.
