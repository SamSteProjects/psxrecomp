...
The read-only scene/actor inspection surface is implemented under `integrations/legaia/inspector`; see `docs/legaia-sdk/town01-inspector.md`. The external headless observer is implemented under `integrations/legaia/observer`; its scoped town01 boundary is live-validated, but retail node traversal remains fail-closed pending exit/re-entry acceptance. RAM writes, actor correlation and generic runtime title checks remain out of scope.
