...
- completed the generic protocol prerequisites for a later observer: protocol 1.2 negotiation, safe runtime/executable identities, authoritative watched-generation reporting, conservative native ownership, bounded frame-stamped `read_regions`, and an eight-record token-bound executable catalog separating images, ranges, and registrations. These changes are generic and contain no Legaia addresses or traversal logic.
- completed native startup correction and live protocol/bounded-read acceptance. The renewed overlay 0897 pass safely retrieved 116 image groups, 116 ranges, and 562 registrations, and explained duplicate static variants, but stopped fail closed because stable town01 field code had no source-matching native owner and the researched base was not an authoritative loaded-image event. See `docs/runtime-launch-acceptance.md` and `docs/legaia-sdk/field-overlay-0897-identity.md`.
...
The read-only scene/actor inspection surface is implemented under `integrations/legaia/inspector`; see `docs/legaia-sdk/town01-inspector.md`. The revisioned research profile is under `integrations/legaia/layouts`; it validates without a disc or runtime. Generic protocol identity, bounded reads, and complete executable-catalog paging now exist, but the profile still fails live selection until field overlay 0897 has a canonical loaded-image and ownership identity. A separately approved headless observer may use these generic commands only after that evidence is recorded. RAM writes, actor correlation and generic runtime title checks remain out of scope.
