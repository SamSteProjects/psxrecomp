...
- implemented the external headless observer, profile-driven scoped epoch, and transition-only manual watch mode. Repeated retail town01 boundary-only passes remained stable despite unrelated global token churn. The real lifecycle was confirmed as `town01 -> map01 -> town0c`; `legaia-na-scus94254-town0c-field-v1` now records the accepted town0c selection. A fresh-process repeat and read-only world-map A/B remain hard gates before any separately approved traversal. See `docs/legaia-sdk/town0c-runtime-profile.md`, `docs/legaia-sdk/world-map-performance-analysis.md`, and `docs/legaia-sdk/actor-observation-readiness.md`.
...
The read-only scene/actor inspection surface is implemented under `integrations/legaia/inspector`; see `docs/legaia-sdk/town01-inspector.md`. The external headless observer is implemented under `integrations/legaia/observer`; its scoped town01 boundary and town0c round-trip transition behavior are live-validated, while retail node traversal remains separately gated. RAM writes, actor correlation and generic runtime title checks remain out of scope.
