    ...
    def _response(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        if command == "protocol_info":
            ...
            if self.scenario == "process_change" and self.runtime_reads >= 3:
                digest = "f" * 64
            result.update({
                "runtime": {"implementation": "psxrecomp", "build_revision": "synthetic"},
                "server": {"kind": "native"},
...
class ObserverTests(unittest.TestCase):
    ...
    def test_13_executable_state_change(self) -> None:
        with self.assertRaises(RetryExhausted): self.capture("executable_change", attempts=1)

    def test_14_lifecycle_token_change(self) -> None:
        with self.assertRaises(RetryExhausted): self.capture("lifecycle_change", attempts=1)
