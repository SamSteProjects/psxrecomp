...
| World-map slowdown attributed to missing compilation without evidence | M | H | Live A/B attributes the measured execution mix primarily to interpreter volume, but MAPDSIP identity and the correct optimization still require separate profiling | world-map-performance-analysis.md |
...
| Internally stable requests combined across unrelated global executable churn | M | H | Protocol 1.5 uses a stateless profile-scoped guard while retaining the global token as a diagnostic; guarded payload is withheld on mismatch | observer/runtime bridge |
| Scoped epoch accepted without a live scene transition test | L | H | Two fresh-process town0c/map01/town0c passes rejected old tokens and recovered stable epochs with zero actor reads/writes; every future traversal must retain the same guards | observer/runtime bridge |
| ISO executable file hash confused with loaded executable-body hash | M | H | Profile records both domains, scopes, ranges and hashes; live selection requires the exact runtime-reported `ps-x-exe-body` identity | profile/provenance |
