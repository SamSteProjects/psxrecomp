...
| Scoped epoch accepted without a live scene transition test | M | H | Transition-only state machine passes synthetic old-token/outside/re-entry gates; keep retail actor traversal disabled until the same proof passes through a normal scene exit using a compatible town01 save | observer/runtime bridge |
