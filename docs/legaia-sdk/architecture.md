...
Build the SDK around one running system—PSXRecomp—and keep Andrew's repository as a pinned engineering reference and validation oracle:

```text
Andrew's docs, parsers, viewers and tests
        |
        | research, attribution, selective adaptation, parity checks
        v
PSXRecomp integrations/legaia/ ----> SDK project model ----> future editor/API
        ^                                  |
        |                                  |
        +---------- JSON/TCP live bridge --+
        ^
        |
user-supplied disc + recompiled Legaia runtime
```

The finished SDK does not require a legend-of-legaia-re checkout, library, subprocess or second runtime. Its current source revision is recorded so discoveries can be traced and refreshed. Compatible parser/analysis logic may be selectively adapted into the isolated Legaia integration with attribution and provenance; it is not copied wholesale. There is no dependency from PSXRecomp's execution loop to the clean-room engine, and no generated MIPS-to-C output goes into legend-of-legaia-re. The future editor talks to the SDK model and never treats live RAM as authored state.
...
| PSXRecomp Legaia integration (`integrations/legaia/`) | disc/asset import needed by the SDK, manifests, schemas, runtime correlation profiles, project model, generated non-proprietary symbol/provenance metadata | copied clean-room engine, extracted game content, hardware special cases |
| legend-of-legaia-re | development reference for typed parsers, scene interpretation, VMs, viewers, patching knowledge, clean-room research and provenance | shipped SDK dependency, generated recomp C, PSXRecomp dispatch internals, SDK authored project truth |
...
integrations/legaia/
  ...
  bridge/          # PSXRecomp TCP client and live correlation
  project-model/   # authored/imported/derived/live/generated types
  generated/       # ignored local build/import results
```

Exact implementation directories remain subject to review after the first importer spike. `external/legend-of-legaia-re/` is intentionally not created: no submodule or vendored checkout is planned.

## Dependency strategy

| Mechanism | Evaluation | Decision |
|---|---|---|
| Git submodule | Would embed a pinned Andrew checkout in this repository | Do not use; record the audited reference commit in provenance instead |
| Cargo git dependency | Would make the SDK/build depend on Andrew's Rust workspace | Do not use for the planned SDK |
...
| Subprocess | Could invoke Andrew's tools, but would make a second project part of the user-facing runtime/toolchain | Not a required architecture; useful only as a development cross-check |
| TCP | Already canonical and safe-pointed in PSXRecomp | Required initial runtime boundary |
| Generated JSON | Human/debuggable and schema-versionable | Use internally for PSXRecomp SDK snapshots, projects and provenance—not as an inter-repository runtime bridge |
...
The initial PSXRecomp-side importer should emit bounded/versioned metadata and optionally content-addressed local mesh buffers. Its implementation should cite the exact Andrew docs/files/commit that informed each adapted parser or semantic rule. Andrew's tools/viewers can be run by developers as parity oracles, but users should not need them to run the SDK.

## Public integration contracts

1. **PSXRecomp-side import contract**: input is user disc path plus scene selector; output is importer/schema version, disc identity, imported scene graph, raw-span locators, semantic claims and diagnostics.
...
2. Ask the PSXRecomp Legaia importer to import one scene and create immutable imported state plus claims.
