...
static int load_one_dll(const char *dll_path,
    ...
    if (s_ndlls >= 0 && s_ndlls < (int)(sizeof(s_dll_image_base) /
                                          sizeof(s_dll_image_base[0]))) {
...
static int candidate_ownership_reason(const Candidate *c, int source_match,
    ...
    if (c->state != ENTRY_VALID || cand_gensum(c) != c->val_gen)
        return source_match ? OVERLAY_OWNERSHIP_GENERATION_INVALIDATED
...
int overlay_loader_get_executable_region(int index, OverlayExecutableRegion *out) {
    ...
    if (index < s_cand_n) {
        ...
        if (c->dll >= 0 && c->dll < (int)(sizeof(s_dll_image_base) /
                                           sizeof(s_dll_image_base[0])) &&
...
static void observer_watch_add(uint32_t *words, uint32_t word_count,
    ...
    if (!words || !word_count || !len || lo >= 0x200000u ||
        len > 0x200000u - lo) return;
